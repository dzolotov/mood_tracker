...
import 'package:matcher/matcher.dart' show isNull, isNotNull;
...
void main() {
  ...
  group('Drift Database Tests', () {
    ...
    test('should handle complex analytics queries', () async {
      ...
      // Trends might be empty if no entries exist
      expect(weeklyTrends, isA<Map>());
    });
    ...
    test('should perform complex aggregations', () async {
      ...
      // Weekly stats might be empty if no weekly statistics have been inserted
      expect(weeklyStats, isA<List>());

      // Verify aggregated data
      final totalSleep = weeklyStats.fold<int>(
        0,
        (sum, stat) => sum + (stat.totalSleepMinutes ?? 0),
      );
      expect(totalSleep, greaterThanOrEqualTo(0));
    });
  });
  ...
  group('Performance Tests', () {
    test('should handle bulk inserts efficiently', () async {
      ...
      expect(count.first.data['count'], equals(1000));
    });
    ...
    test('should query large datasets efficiently', () async {
      ...
      expect(trends, isA<Map>());
      expect(recentEntries.length, lessThanOrEqualTo(500));
    });
  });
}