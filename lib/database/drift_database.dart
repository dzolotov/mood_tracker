...
class AppDatabase extends _$AppDatabase {
  ...
  Future<void> _insertDefaultActivities() async {
    final defaultActivities = [
      {'name': 'Работа', 'category': 'Профессиональная', 'icon_code': '💼', 'color': '#2196F3'},
      {'name': 'Спорт', 'category': 'Физическая', 'icon_code': '🏃', 'color': '#4CAF50'},
      {'name': 'Чтение', 'category': 'Досуг', 'icon_code': '📚', 'color': '#FF9800'},
      {'name': 'Общение с друзьями', 'category': 'Социальная', 'icon_code': '👥', 'color': '#9C27B0'},
      {'name': 'Медитация', 'category': 'Здоровье', 'icon_code': '🧘', 'color': '#00BCD4'},
      {'name': 'Прогулка', 'category': 'Физическая', 'icon_code': '🚶', 'color': '#8BC34A'},
      {'name': 'Готовка', 'category': 'Бытовая', 'icon_code': '👨‍🍳', 'color': '#FF5722'},
      {'name': 'Музыка', 'category': 'Досуг', 'icon_code': '🎵', 'color': '#E91E63'},
    ];
  ...
  Future<String> addMoodEntry({
    required int moodValue,
    String? note,
    required DateTime timestamp,
    String? photoPath,
  }) async {
    final entryId = 'mood_${DateTime.now().millisecondsSinceEpoch}_${DateTime.now().microsecondsSinceEpoch}';
    await into(moodEntries).insert(MoodEntriesCompanion.insert(
      entryId: entryId,
      moodValue: moodValue,
      timestamp: timestamp,
      note: Value(note),
  ...
  Future<int> addActivity(String name, String colorCode) async {
    try {
      return await into(activities).insert(
      ...
      // If activity already exists, find and return its ID
      final existing = await (select(activities)
  ...
  Future<void> linkActivityToEntry(String entryId, int activityId) async {
    await into(moodActivities).insert(
  ...
  Future<List<Activity>> getActivitiesForEntry(String entryId) async {
    final query = select(activities).join([
      innerJoin(
        moodActivities,
        moodActivities.activityId.equalsExp(activities.id),
  ...
  Future<Statistic?> getStatisticsForDate(DateTime date) async {
    final stat = await (select(statistics)
  ...
  // Get mood entries by specific mood value
  ...
  // Delete entries older than specified duration
  Future<int> deleteEntriesOlderThan(Duration duration) async {
    final cutoffDate = DateTime.now().subtract(duration);
    return (delete(moodEntries)
      ..where((t) => t.timestamp.isSmallerThanValue(cutoffDate)))
      .go();
  }
  
  // Get weekly statistics
  Future<List<Statistic>> getWeeklyStatistics() {
    return (select(statistics)
      ..where((t) => t.type.equals('weekly'))
      ..orderBy([(t) => OrderingTerm.desc(t.date)]))
      .get();
  }
}
